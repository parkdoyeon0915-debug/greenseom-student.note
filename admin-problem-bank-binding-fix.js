...
const script=`<script id="admin-problem-bank-binding-fix-v4">(function(){
function getId(row){return Number(row.dataset.studentId||0);}
...
function renderPanel(panel,p){
  ...
  schools.forEach(s=>{
    ...
    if(!entries.length){html+='<div class="muted">아직 진행 기록이 없습니다.</div>';}else{
      html+='<div class="pb-prompts">'+entries.map(k=>{const v=status[k]||'미진행';return '<div class="pb-prompt '+cls(v)+'"><span>'+esc(k.slice(s.length+2))+'</span><b>'+esc(v)+'</b></div>';}).join('')+'</div>';
    }
    html+='</div>';
  });
...
function addButtons(){
  document.querySelectorAll('#students .student').forEach(row=>{
    ...
    btn.addEventListener('click',async e=>{
      ...
      try{
        const id=Number(btn.dataset.studentId);const r=await fetch('/api/admin/problem-bank/'+encodeURIComponent(id),{credentials:'same-origin',cache:'no-store'});const p=await r.json();
...
function boot(){
  ...
  if(root&&!root.dataset.pbV4Observer){root.dataset.pbV4Observer='1';new MutationObserver(()=>setTimeout(addButtons,0)).observe(root,{childList:true,subtree:true});}
...
console.log('GREENSUM admin problem bank binding v4 loaded');
