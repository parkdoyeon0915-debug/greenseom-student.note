...
function sendBrandedPage(file,res){
  ...
  if(file==="index.html"){
    html=html.replace("</aside>",`<a href="/problem-bank.html" class="problem-bank-link" id="problemBankNav">📚 문제은행</a></aside>`);
    html=html.replace("</head>",`<style id="problem-bank-nav-style">.problem-bank-link{display:block;padding:12px;border-radius:10px;font-weight:800;cursor:pointer;margin:3px 0;text-decoration:none;color:inherit}.problem-bank-link:hover{background:#edf3f9}@media(max-width:900px){.side{display:flex;align-items:center;gap:8px;overflow-x:auto;overflow-y:hidden;white-space:nowrap;-webkit-overflow-scrolling:touch}.side .problem-bank-link{flex:0 0 auto;width:auto;min-height:48px;white-space:nowrap;margin:0;padding:12px 16px}}</style></head>`);
