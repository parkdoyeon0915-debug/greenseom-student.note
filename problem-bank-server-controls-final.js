...
function mount(){if(document.getElementById('problemBankServerControls'))return;const hero=document.querySelector('.hero');if(!hero)return;const b=document.createElement('div');b.id='problemBankServerControls';b.style='display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-top:18px;padding-top:14px;border-top:1px solid #e4e8ec';const l=document.createElement('button');l.type='button';l.className='btn';l.textContent='☁ 서버 불러오기';l.onclick=loadServer;const s=document.createElement('button');s.type='button';s.className='btn primary';s.textContent='☁ 서버 저장';s.onclick=saveServer;const st=document.createElement('span');st.id='pbServerState';st.textContent='현재 학생 ID: '+(id||'로그인 학생');st.style='font-size:12px;font-weight:800;margin-left:auto;color:#7d8791';b.append(l,s,st);hero.appendChild(b)}
function boot(){mount();setTimeout(loadServer,100)}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot);else boot();new MutationObserver(mount).observe(document.documentElement,{childList:true,subtree:true});
  ...
  express.response.send=function(body){if(typeof body==='string'&&this.req&&this.req.path==='/problem-bank.html'&&body.includes('</body>'))body=body.replace('</body>',script+'</body>');return originalSend.call(this,body)};
  express.response.__greensumProblemBankFinalControls=true;
}
console.log('GREENSUM final problem bank controls loaded');
