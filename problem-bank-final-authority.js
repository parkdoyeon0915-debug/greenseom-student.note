...
function apply(d){set.call(localStorage,sc(SK),JSON.stringify(d.schools||['','','']));for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i);if(k&&k.startsWith(SP)&&k.endsWith(sf))rem.call(localStorage,k)}Object.entries(d.status||{}).forEach(([k,v])=>set.call(localStorage,sc(SP+k),v));window.load?.();window.render?.();window.renderGallery?.()}
