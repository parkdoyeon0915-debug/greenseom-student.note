...
app.put("/api/diagnoses/:id",login,upload.single("photo"),async(req,res)=>{try{const old=(await q("SELECT * FROM diagnoses WHERE id=$1 AND user_id=$2",[req.params.id,req.session.user.id])).rows[0];if(!old)return res.status(404).json({error:"기록을 찾을 수 없습니다."});const b=req.body;const f=["problem_analysis","form_score","completion","expression","composition"].map(k=>Math.max(0,Math.min(5,+b[k]||0)));const x=await q(`UPDATE diagnoses SET date=$1,subject=$2,photo_data=$3,photo_mime=$4,problem_analysis=$5,form_score=$6,completion=$7,expression=$8,composition=$9,notes=$10,improve=$11 WHERE id=$12 RETURNING *`,[b.date,b.subject||"",req.file?.buffer||old.photo_data,req.file?.mimetype||old.photo_mime,...f,b.notes||"",b.improve||"",old.id]);res.json(diagOut(x.rows[0]))}catch(e){console.error("PUT diagnoses",e);res.status(500).json({error:"자가진단 수정 저장에 실패했습니다."})}});
app.delete("/api/diagnoses/:id",login,async(req,res)=>{try{const r=(await q("DELETE FROM diagnoses WHERE id=$1 AND user_id=$2 RETURNING id",[req.params.id,req.session.user.id])).rows[0];if(!r)return res.status(404).json({error:"기록을 찾을 수 없습니다."});res.json({ok:true})}catch(e){console.error("DELETE diagnoses",e);res.status(500).json({error:"삭제에 실패했습니다."})}});

app.get("/api/patterns",login,async(req,res)=>{try{const ps=(await q("SELECT * FROM patterns WHERE user_id=$1 ORDER BY id DESC",[req.session.user.id])).rows;const out=[];for(const p of ps){const imgs=(await q("SELECT id,photo_data FROM pattern_images WHERE pattern_id=$1 ORDER BY id",[p.id])).rows.map(x=>({...x,photo:x.photo_data?photoUrl("pattern-images",x.id):null}));out.push({...patternOut(p),images:imgs})}res.json(out)}catch(e){console.error("GET patterns",e);res.status(500).json({error:"패턴 기록을 불러오지 못했습니다."})}});
app.post("/api/patterns",login,upload.fields([{name:"photo",maxCount:1},{name:"images"}]),async(req,res)=>{try{const b=req.body;const main=req.files?.photo?.[0];const x=await q("INSERT INTO patterns(user_id,name,photo_data,photo_mime,must_keep,cautions,self_feedback) VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING *",[req.session.user.id,b.name||"새 패턴",main?.buffer||null,main?.mimetype||null,b.must_keep||"",b.cautions||"",b.self_feedback||""]);for(const f of(req.files?.images||[]))await q("INSERT INTO pattern_images(pattern_id,photo_data,photo_mime) VALUES($1,$2,$3)",[x.rows[0].id,f.buffer,f.mimetype]);const row=x.rows[0];const images=(await q("SELECT id,photo_data FROM pattern_images WHERE pattern_id=$1 ORDER BY id",[row.id])).rows.map(i=>({...i,photo:i.photo_data?photoUrl("pattern-images",i.id):null}));res.status(201).json({...patternOut(row),images})}catch(e){console.error("POST patterns",e);res.status(500).json({error:"패턴 서버 저장에 실패했습니다."})}});
app.put("/api/patterns/:id",login,upload.fields([{name:"photo",maxCount:1},{name:"images"}]),async(req,res)=>{try{const old=(await q("SELECT * FROM patterns WHERE id=$1 AND user_id=$2",[req.params.id,req.session.user.id])).rows[0];if(!old)return res.status(404).json({error:"패턴을 찾을 수 없습니다."});const b=req.body;const main=req.files?.photo?.[0];const x=await q("UPDATE patterns SET name=$1,photo_data=$2,photo_mime=$3,must_keep=$4,cautions=$5,self_feedback=$6 WHERE id=$7 RETURNING *",[b.name||"새 패턴",main?.buffer||old.photo_data,main?.mimetype||old.photo_mime,b.must_keep||"",b.cautions||"",b.self_feedback||"",old.id]);for(const f of(req.files?.images||[]))await q("INSERT INTO pattern_images(pattern_id,photo_data,photo_mime) VALUES($1,$2,$3)",[old.id,f.buffer,f.mimetype]);const row=x.rows[0];const images=(await q("SELECT id,photo_data FROM pattern_images WHERE pattern_id=$1 ORDER BY id",[row.id])).rows.map(i=>({...i,photo:i.photo_data?photoUrl("pattern-images",i.id):null}));res.json({...patternOut(row),images})}catch(e){console.error("PUT patterns",e);res.status(500).json({error:"패턴 수정 저장에 실패했습니다."})}});
app.delete("/api/patterns/:id",login,async(req,res)=>{try{const r=(await q("DELETE FROM patterns WHERE id=$1 AND user_id=$2 RETURNING id",[req.params.id,req.session.user.id])).rows[0];if(!r)return res.status(404).json({error:"패턴을 찾을 수 없습니다."});res.json({ok:true})}catch(e){console.error("DELETE patterns",e);res.status(500).json({error:"패턴 삭제에 실패했습니다."})}});

app.get("/api/files/diagnoses/:id",login,async(req,res)=>{try{const r=(await q("SELECT photo_data,photo_mime,user_id FROM diagnoses WHERE id=$1",[req.params.id])).rows[0];if(!r||r.user_id!==req.session.user.id&&req.session.user.role!=="admin")return res.status(404).end();if(!r.photo_data)return res.status(404).end();res.type(r.photo_mime||"image/jpeg").send(r.photo_data)}catch(e){console.error("file diagnoses",e);res.status(500).end()}});
app.get("/api/files/patterns/:id",login,async(req,res)=>{try{const r=(await q("SELECT photo_data,photo_mime,user_id FROM patterns WHERE id=$1",[req.params.id])).rows[0];if(!r||r.user_id!==req.session.user.id&&req.session.user.role!=="admin")return res.status(404).end();if(!r.photo_data)return res.status(404).end();res.type(r.photo_mime||"image/jpeg").send(r.photo_data)}catch(e){console.error("file patterns",e);res.status(500).end()}});
app.get("/api/files/pattern-images/:id",login,async(req,res)=>{try{const r=(await q("SELECT pi.photo_data,pi.photo_mime,p.user_id FROM pattern_images pi JOIN patterns p ON p.id=pi.pattern_id WHERE pi.id=$1",[req.params.id])).rows[0];if(!r||r.user_id!==req.session.user.id&&req.session.user.role!=="admin")return res.status(404).end();if(!r.photo_data)return res.status(404).end();res.type(r.photo_mime||"image/jpeg").send(r.photo_data)}catch(e){console.error("file pattern-images",e);res.status(500).end()}});

app.get("/api/admin/students",admin,async(req,res)=>{try{const rows=(await q(`SELECT u.id,u.username,u.name,COUNT(DISTINCT d.id)::int diagnoses,COUNT(DISTINCT p.id)::int patterns,MAX(d.date) last_date FROM users u LEFT JOIN diagnoses d ON d.user_id=u.id LEFT JOIN patterns p ON p.user_id=u.id WHERE u.role='student' GROUP BY u.id ORDER BY u.name`)).rows;res.json(rows)}catch(e){console.error("admin students",e);res.status(500).json({error:"학생 목록을 불러오지 못했습니다."})}});
app.get("/api/admin/students/:id",admin,async(req,res)=>{try{const u=(await q("SELECT id,username,name FROM users WHERE id=$1 AND role='student'",[req.params.id])).rows[0];if(!u)return res.status(404).json({error:"학생을 찾을 수 없습니다."});const ds=(await q("SELECT * FROM diagnoses WHERE user_id=$1 ORDER BY date DESC,id DESC",[u.id])).rows;const ps=(await q("SELECT * FROM patterns WHERE user_id=$1 ORDER BY id DESC",[u.id])).rows;const patterns=[];for(const p of ps){const images=(await q("SELECT id,photo_data FROM pattern_images WHERE pattern_id=$1 ORDER BY id",[p.id])).rows.map(i=>({...i,photo:i.photo_data?photoUrl("pattern-images",i.id):null}));patterns.push({...patternOut(p),images})}res.json({student:u,diagnoses:ds.map(diagOut),patterns})}catch(e){console.error("admin student",e);res.status(500).json({error:"학생 기록을 불러오지 못했습니다."})}});
app.get("/api/admin/diagnoses/:id",admin,async(req,res)=>{try{const r=(await q("SELECT d.*,u.name,u.username FROM diagnoses d JOIN users u ON u.id=d.user_id WHERE d.id=$1 AND u.role='student'",[req.params.id])).rows[0];if(!r)return res.status(404).json({error:"기록을 찾을 수 없습니다."});res.json({...diagOut(r),student:{name:r.name,username:r.username}})}catch(e){console.error("admin diagnosis",e);res.status(500).json({error:"기록을 불러오지 못했습니다."})}});
app.put("/api/admin/diagnoses/:id",admin,async(req,res)=>{try{const r=(await q("SELECT d.id FROM diagnoses d JOIN users u ON u.id=d.user_id WHERE d.id=$1 AND u.role='student'",[req.params.id])).rows[0];if(!r)return res.status(404).json({error:"기록을 찾을 수 없습니다."});await q("UPDATE diagnoses SET teacher_note=$1 WHERE id=$2",[String(req.body.teacher_note||""),r.id]);res.json({ok:true})}catch(e){console.error("admin teacher note",e);res.status(500).json({error:"코멘트 저장에 실패했습니다."})}});

app.use((err,req,res,next)=>{console.error("UNHANDLED",err);if(res.headersSent)return next(err);if(err instanceof multer.MulterError){const msg=err.code==='LIMIT_FILE_SIZE'?'사진 파일이 너무 큽니다. 10MB 이하로 올려주세요.':`사진 업로드 오류: ${err.message}`;return res.status(400).json({error:msg})}res.status(500).json({error:"서버에서 오류가 발생했습니다. 잠시 후 다시 시도해주세요."})});
