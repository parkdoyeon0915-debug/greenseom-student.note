...
function sendBrandedPage(file,res){
  ...
  if(file==="index.html"){
    // Always render exactly ONE problem-bank navigation item. Remove any older
    // injected copies first so layered fixes cannot create duplicates.
    html=html.replace(/<[^>]*class=["'][^"']*\\bnav\\b[^"']*["'][^>]*>[^<]*📚\\s*문제은행[^<]*<\\/[^>]+>/g,"");
    const problemNav=`<a href="/problem-bank.html" class="nav" id="problemBankNav" data-page="problemBank">📚 문제은행</a>`;
    html=html.replace("</aside>",problemNav+"</aside>");
    html=html.replace("</head>",`<style id="problem-bank-nav-style">@media(max-width:900px){.side{display:flex;align-items:center;gap:8px;overflow-x:auto;overflow-y:hidden;white-space:nowrap;-webkit-overflow-scrolling:touch}.side .nav{flex:0 0 auto;width:auto;min-height:48px;white-space:nowrap;margin:0;padding:12px 16px}}</style></head>`);
    html=html.replace("</body>",`<script>(function(){
      function bindProblemBank(){
        var nav=document.getElementById('problemBankNav');
        if(!nav||nav.dataset.bound==='1')return;
        nav.dataset.bound='1';
        nav.addEventListener('click',function(){
          document.querySelectorAll('.nav.active').forEach(function(n){n.classList.remove('active')});
...
app.get("/problem-bank.html",(req,res)=>{if(!req.session.user)return res.redirect("/");res.set("Cache-Control","no-store, no-cache, must-revalidate, proxy-revalidate");res.type("html").send(fs.readFileSync(path.join(ROOT,"public","problem-bank.html"),"utf8").replaceAll("GREENSEOM","GREENSUM"))});
...
app.get("/api/files/:type/:id",login,async(req,res)=>{const allowed={diagnoses:"photo_data",patterns:"photo_data",pattern_images:"photo_data"};const col=allowed[req.params.type];if(!col)return res.status(404).end();const table=req.params.type;try{const r=await q(`SELECT ${col},photo_mime FROM ${table} WHERE id=$1`,[req.params.id]);if(!r.rowCount||!r.rows[0][col])return res.status(404).end();res.type(r.rows[0].photo_mime||"image/jpeg").send(r.rows[0][col])}catch(e){res.status(500).end()}});
app.get("/api/admin/students",admin,async(req,res)=>{const rows=(await q("SELECT id,username,name,created_at FROM users WHERE role='student' ORDER BY name,id")).rows;res.json(rows)});
app.get("/api/admin/students/:id",admin,async(req,res)=>{const u=(await q("SELECT id,username,name,created_at FROM users WHERE id=$1",[req.params.id])).rows[0];if(!u)return res.status(404).json({error:"학생을 찾을 수 없습니다."});const d=(await q("SELECT * FROM diagnoses WHERE user_id=$1 ORDER BY date DESC,id DESC",[req.params.id])).rows.map(diagOut);const p=(await q("SELECT * FROM patterns WHERE user_id=$1 ORDER BY id DESC",[req.params.id])).rows;for(const r of p)r.images=(await q("SELECT id FROM pattern_images WHERE pattern_id=$1 ORDER BY id",[r.id])).rows.map(x=>({id:x.id,photo:photoUrl("pattern_images",x.id)}));res.json({student:u,diagnoses:d,patterns:p.map(patternOut)})});
app.get("/api/admin/diagnoses/:id",admin,async(req,res)=>{const r=(await q("SELECT d.*,u.name AS student_name FROM diagnoses d JOIN users u ON u.id=d.user_id WHERE d.id=$1",[req.params.id])).rows[0];if(!r)return res.status(404).json({error:"기록을 찾을 수 없습니다."});res.json({...diagOut(r),student:{name:r.student_name}})});
app.put("/api/admin/diagnoses/:id",admin,async(req,res)=>{const r=(await q("UPDATE diagnoses SET teacher_note=$1 WHERE id=$2 RETURNING *",[req.body.teacher_note||"",req.params.id])).rows[0];if(!r)return res.status(404).json({error:"기록을 찾을 수 없습니다."});res.json(diagOut(r))});
app.get("/api/admin/diagnoses",admin,async(req,res)=>{const rows=(await q("SELECT d.*,u.name AS student_name FROM diagnoses d JOIN users u ON u.id=d.user_id ORDER BY d.date DESC,d.id DESC")).rows;res.json(rows.map(r=>({...diagOut(r),student:{name:r.student_name}})))});
app.get("/api/admin/patterns",admin,async(req,res)=>{const rows=(await q("SELECT p.*,u.name AS student_name FROM patterns p JOIN users u ON u.id=p.user_id ORDER BY p.id DESC")).rows;res.json(rows.map(r=>({...patternOut(r),student:{name:r.student_name}})))});
app.get("/api/admin/diagnoses/:id/comments",admin,async(req,res)=>{const r=(await q("SELECT teacher_note FROM diagnoses WHERE id=$1",[req.params.id])).rows[0];res.json({comments:r?.teacher_note||""})});
app.post("/api/admin/diagnoses/:id/comments",admin,async(req,res)=>{const r=(await q("UPDATE diagnoses SET teacher_note=$1 WHERE id=$2 RETURNING *",[req.body.comment||"",req.params.id])).rows[0];if(!r)return res.status(404).json({error:"기록을 찾을 수 없습니다."});res.json(diagOut(r))});
