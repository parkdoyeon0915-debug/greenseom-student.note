...
// Final admin problem-bank binding: remove earlier button listeners and render
// whatever the server actually returns, even when updated_at is missing.
...
function render(panel,p){
  ...
  schools.forEach(s=>{
    const entries=keys.filter(k=>k.indexOf(s+'::')===0);
  ...
  const unknown=keys.filter(k=>!schools.some(s=>k.indexOf(s+'::')===0));
  if(unknown.length){html+='<div class="pb-school"><div class="pb-school-title">저장된 기타 진행 기록</div><div class="pb-prompts">'+unknown.map(k=>{const v=status[k]||'미진행';return '<div class="pb-prompt '+cls(v)+'"><span>'+esc(k)+'</span><b>'+esc(v)+'</b></div>';}).join('')+'</div></div>';}
...
async function openProgress(row,btn){
  ...
  try{
    ...
    const r=await fetch('/api/admin/problem-bank/'+encodeURIComponent(id),{credentials:'same-origin',cache:'no-store'});
...
function bind(){
  document.querySelectorAll('#students .student').forEach(row=>{
    ...
    let old=row.querySelector('.pb-progress-btn');
    if(!old){const actions=row.lastElementChild;if(!actions)return;old=document.createElement('button');old.type='button';old.className='btn pb-progress-btn';old.textContent='문제은행 진도';actions.appendChild(old);}
    // Clone to remove every earlier click listener registered by older fixes.
    if(old.dataset.finalBound!=='1'){
      const btn=old.cloneNode(true);btn.dataset.studentId=String(id);btn.dataset.finalBound='1';old.replaceWith(btn);
      btn.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();openProgress(row,btn)},true);
    }else if(!old.dataset.studentId){old.dataset.studentId=String(id);}
  });
}
function boot(){
  bind();
  const root=document.getElementById('students');
  if(root&&!root.dataset.pbFinalObserver){root.dataset.pbFinalObserver='1';new MutationObserver(()=>setTimeout(bind,0)).observe(root,{childList:true,subtree:true});}
  setTimeout(bind,50);setTimeout(bind,300);setTimeout(bind,1000);setTimeout(bind,2000);
}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot);else boot();
})();</script>`;
express.response.send=function(body){
  if(typeof body==='string'&&this.req&&this.req.path==='/admin.html'&&body.includes('</body>'))body=body.replace('</head>',finalProblemBankAdminStyle+'</head>').replace('</body>',finalProblemBankAdminScript+'</body>');
  return originalProblemBankAdminSend.call(this,body);
};
console.log('GREENSUM admin problem bank final fix loaded');
